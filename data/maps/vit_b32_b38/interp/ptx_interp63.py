...
"""launch63 PTX address/provenance interpreter (derived from ../r03-launch61/ptx_interp61.py and ../r02-launch60/ptx_interp60.py, unchanged).

Executes every thread of gridDim 8x1x4 (ctaid.x, ctaid.z) x blockDim 32x4x1 (lane, warp) = 4,096 threads of
cc_vit_1d_projection_chained_fp8.  Integer/address instructions exactly; data symbolically (hash-consed DAG, no numbers).
Kinds: sread (shared input), gload (weights), g32 (scale words), cg (residual), dq (e4m3 -> f16), bufstore (z0), red (REDG, z1/z2),
bufload (z3), outstore (z3).  Joint hypotheses: H_TID_X_ZERO, H_ELECT_ANY, H_BARRIER_VIS, H_BRACE_LE, H_STAGE_ORDER
(stage z of a ctaid.x runs after stage z-1 wrote release63[4*ctaid.x]; the predictor replays z0 -> z3 in that order).
Output: maps/l63-address-map.npz, maps/l63-templates-and-checks.json.
"""
import sys
...
FUNC = _arg('--func', 'cc_vit_1d_projection_chained_fp8')
...
BUFFER_BYTES = 131072
LEAF_KINDS = ('sread', 'gload', 'g32', 'cg', 'bufload')
STORE_KINDS = ('bufstore', 'red', 'outstore')
    ...
    def run(self, max_steps=600000):
        ...
        while pc < n:
            ...
            if mn.startswith('mov.') and mn != MMA:
                ...
                kind = {'weights': 'gload', 'buffer': 'bufload', 'residual': 'cg'}.get(reg)
                if kind is None:
                ...
                kind = {'buffer': 'bufstore', 'output': 'outstore'}.get(reg)
                if kind is None:
                ...
                if reg != 'input':
                    raise Unsupported('copy source outside launch63 input')
...
def main():
    ...
    WB = s0['weights']['bytes']
    regions = dict(input=(w[0], 65536), residual=(w[1], 65536), output=(w[2], 65536), weights=(w[3], WB), release63=(w[4], 128),
                   buffer=(w[5], BUFFER_BYTES), release62=(w[6], 128), release64=(w[7], 128))
    ...
    buf = lambda k: 32768 * (k[2] // 4) + 512 * (k[2] % 4) + 2048 * pp + 4096 * cc + 16 * ll  # noqa: E731
    cmp('sread_src', 'sread', lambda k: np.where(live, 16384 * k[2] + 4096 * zz + 512 * k[1] + 16 * ll, -1))
    cmp('gload', 'gload', lambda k: 32768 * (k[1] + 1) + 262144 * zz + 2048 * pp + 4096 * cc + 512 * k[2] + 16 * ll)
    cmp('g32_scale_z0', 'g32', lambda k: np.where(zz == 0, 1048576 + 128 * pp + 256 * cc + 16 * ((k[2] % 16) // 2) + 4 * (ll % 4), -1))
    cmp('cg_residual_z0', 'cg', lambda k: np.where((zz == 0) & live, 16384 * (k[2] // 2) + 512 * (k[2] % 2) + 1024 * pp + 2048 * cc + 16 * ll, -1))
    cmp('bufstore_z0', 'bufstore', lambda k: np.where((zz == 0) & live, buf(k), -1))
    cmp('red_z1_z2', 'red', lambda k: np.where(((zz == 1) | (zz == 2)) & live, buf(k), -1))
    cmp('bufload_z3', 'bufload', lambda k: np.where((zz == 3) & live, buf(k), -1))
    cmp('outstore_z3', 'outstore', lambda k: np.where((zz == 3) & live, 16384 * (k[2] // 2) + 512 * (k[2] % 2) + 1024 * pp + 2048 * cc + 16 * ll, -1))
    ...
    def cover(kind, size, zsel, width_=16):
        ...
        for k in keys:
            ...
            for b in range(width_):
                np.add.at(cnt, a + b, 1)
        return cnt
    cov = dict(output_exactly_once=[int((cover('outstore', 65536, [3]) == 1).sum()), 65536],
               buffer_bufstore_z0_exactly_once=[int((cover('bufstore', BUFFER_BYTES, [0]) == 1).sum()), BUFFER_BYTES],
               buffer_red_z1_exactly_once=[int((cover('red', BUFFER_BYTES, [1]) == 1).sum()), BUFFER_BYTES],
               buffer_red_z2_exactly_once=[int((cover('red', BUFFER_BYTES, [2]) == 1).sum()), BUFFER_BYTES],
               buffer_bufload_z3_exactly_once=[int((cover('bufload', BUFFER_BYTES, [3]) == 1).sum()), BUFFER_BYTES],
               residual_read_exactly_once_z0=[int((cover('cg', 65536, [0]) == 1).sum()), 65536],
               weights_matrix_read_exactly_once_by_live_warps=[int((cover('gload', 1048576, [0, 1, 2, 3])[:1048576] == 1).sum()), 1048576])
    cov['input_bytes_read_counts_live'] = sorted(set(cover('sread', 65536, [0, 1, 2, 3]).tolist()))
    cov['scale_words_read_counts_z0'] = sorted(set(cover('g32', WB, [0], 4)[1048576:1048576 + 2048].tolist()))
    ...
    cov_ok = all(v[0] == v[1] for k, v in cov.items() if isinstance(v, list) and len(v) == 2 and isinstance(v[0], int) and k.endswith(('once', 'warps', 'once_z0')))
    ok = (sum(fails.values()) == 0 and sum(replay.values()) == 0 and tidx_independent and closed_ok and cov_ok
          and all(len(v) == 8 and all(n == 32 for n in v.values()) for v in mma_per_epoch.values()))
    const_rows = [dict(pc=k[0], operand=k[1], f16=k[2], exact=k[3], count=n) for k, n in sorted(consts.items(), key=lambda q: (q[0][0], q[0][1]))]
    rep = dict(format='opus-b31-l63-address-map/1', valid=ok, match=None, status='PASS_EXECUTABLE_MAP' if ok else 'MAP_CHECK_FAILED',
               ...
               epoch_marks=sorted(marks), mma_per_epoch_live=mma_per_epoch, closed_form_cross_check=closed, coverage=cov, constants=const_rows,
               ...
               what_a_false_pass_would_look_like={
                   ...
                   'ptx_arithmetic_differs_from_executed_sass': 'NOT caught here; see sass_family_check_l63.py'},
    ...
    print(json.dumps({k: rep[k] for k in ('status', 'interpreter_failures', 'shared_replay', 'tid_x_independence_probe', 'backward_branches_taken',
                                          'mma_per_epoch_live', 'closed_form_cross_check', 'coverage', 'releases', 'integer_bit_constants_used_as_data',
                                          'seconds')}, indent=1))
