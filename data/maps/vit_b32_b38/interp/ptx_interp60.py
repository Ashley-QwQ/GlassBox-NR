...
"""launch60 PTX address/provenance interpreter (derived from ../../ptx_interp.py, which is unchanged).

Executes every thread of gridDim 8x1x4 (ctaid.x, ctaid.z) x blockDim 32x4x1 (lane, warp) = 4,096 threads of
cc_vit_1d_ffn_contract_chained_fp8.  Integer/address instructions exactly; data symbolically.  Adds, relative to launch59:
  ctaid.z stages, or.pred/not/min.s32, cvt.s64.s32, shl.b64, [reg+const] memory, ld.global.b32 (scale words),
  ld.weak.global.cg (residual), dest-group mov {rs,rs} <- b32, cvt e4m3->f16 (dq), add.f16x2,
  red.global.v4.f16x2.add.noftz (REDG), ld.relaxed.gpu release reads (policy: completed), nanosleep (must not execute).
Epochs: loop head (see the interpreter's own --loop-head argument) plus the statement after the loop back-edge (the 16th, unrolled epoch).
Joint hypotheses (as launch59): H_TID_X_ZERO, H_ELECT_ANY, H_BARRIER_VIS, H_BRACE_LE; new:
  H_STAGE_ORDER  stage z of a given ctaid.x runs after stage z-1 has written its release word (the PTX wait reads
                 release[ctaid.x] >= z-1 and every stage writes release[ctaid.x] = z at its end); the buffer replay uses this order.
Output: maps/l60-address-map.npz (per-event offsets), maps/l60-templates-and-checks.json.
"""
import sys
...
FUNC = _arg('--func', 'cc_vit_1d_ffn_contract_chained_fp8')
...
def f32_to_f16(bits):
    ...
    if float(np.float16(v)) != v:
        raise Unsupported('f32 constant not exact in binary16')
    return v
    ...
    def run(self, max_steps=400000):
        ...
        while pc < n:
            ...
            if mn.startswith('mov.'):
                ...
                self.put(ops[0], s32(self.ival(ops[1])))
            ...
            elif mn in ('ld.weak.global.ca.v4.u32', 'ld.weak.global.cg.v4.u32'):
                reg, off = self.region(self.addr(ops[1]), 16)
                kind = {'weights': 'gload', 'buffer': 'bufload', 'residual': 'cg'}[reg]
                ...
                for i, r in enumerate(d[1]):
                    self.R[r[1]] = ('ev', e, i)
            elif mn in ('mul.f16x2', 'add.f16x2'):
                self.R[ops[0][1]] = ('fop', mn.split('.')[0], (self.val(ops[1]), self.val(ops[2])))
            elif mn == 'cvt.rn.f16.f32':
                self.R[ops[0][1]] = ('c16', f32_to_f16(self.ival(ops[1])))
            elif mn == 'cvt.rn.satfinite.e4m3x2.f16x2':
                self.R[ops[0][1]] = ('sat', self.val(ops[1]))
            elif mn == 'cvt.rn.f16x2.e4m3x2':
                self.R[ops[0][1]] = ('dq', self.val(ops[1]))
            elif mn == 'st.global.L1::no_allocate.b128':
                reg, off = self.region(self.addr(ops[0]), 16)
                self.emit({'buffer': 'bufstore', 'output': 'outstore'}[reg], here, off, 16, self.val(ops[1]))
            elif mn == 'red.global.v4.f16x2.add.noftz':
                reg, off = self.region(self.addr(ops[0]), 16)
                if reg != 'buffer':
                    raise Unsupported('REDG outside buffer')
                self.emit('red', here, off, 16, self.val(ops[1]))
            elif mn == 'cp.async.bulk.shared::cta.global.mbarrier::complete_tx::bytes':
                reg, off = self.region(self.ival(ops[1][1]), self.ival(ops[2]))
                if reg != 'input':
                    raise Unsupported('copy source outside launch60 input')
                ...
                self.emit('release', here, reg, off, self.ival(ops[1]))
            elif mn == 'ret':
                return
            else:
...
def main():
    ...
    if grid != [8, 1, 4] or block != [32, 4, 1]:
        raise SystemExit('unexpected geometry')
    w = struct.unpack('<9Q', params)
    regions = dict(input=(w[0], 262144), residual=(w[1], 65536), output=(w[2], 65536), weights=(w[3], s0['weights']['bytes']),
                   release60=(w[4], 32), buffer=(w[5], 131072), release61=(w[7], 32),
                   release59=(w[6], 128))   # launch59 writes one release word per launch59 CTA (32 x 4 bytes)
    ...
    def canon_thread(th):
        ...
        for ev in th.events:
            kind, ep, pc, eid = ev[:4]
            if kind in ('sread', 'gload', 'cg', 'g32', 'bufload', 'mma', 'bufstore', 'red', 'outstore'):
        ...
        def canon(sym):
            ...
            k = sym[0]
            if k == 'ev':
                return slot[sym[1]] + (sym[2],)
            if k == 'c16':
                return ('c16', repr(float(sym[1])))
            if k in ('sat', 'dq'):
                return (k, canon(sym[1]))
            if k == 'part':
                return ('part', canon(sym[1]), sym[2])
            if k == 'cat':
                return ('cat',) + tuple(canon(x) for x in sym[1])
            if k == 'fop':
                return ('fop', sym[1]) + tuple(canon(x) for x in sym[2])
        ...
        for ev in th.events:
            ...
            elif kind in ('bufstore', 'red', 'outstore'):
                stores.append((kind, slot[eid][2], pc, canon(ev[6])))
            ...
            for y in range(NY):
                for lane in range(NL):
                    ...
                    for ev in th.events:
                        kind, ep, pc, eid = ev[:4]
                        ev_counts[kind] += 1
                        if kind in ('sread', 'gload', 'cg', 'g32', 'bufload', 'bufstore', 'red', 'outstore'):
                            ...
                            releases[(z, y, ev[4], ev[5], ev[6] if ev[4] != 'release61' else ev[6])] += 1
                        elif kind == 'relread':
                            relreads[(z, ep, ev[4], ev[5] - 4 * c if ev[4] == 'release60' else ev[5])] += 1
                    wz = waits_by_z.setdefault(z, waits)
                    if wz != waits:
                        fails['wait_sequence_differs_within_stage'] += 1
                    wiring, stores = canon_thread(th)
                    cls = (z, 'live' if y < 2 else 'dead')
                    if cls not in templates:
                        templates[cls] = dict(wiring=wiring, stores=stores)
                    else:
                        if templates[cls]['wiring'] != wiring:
                            fails['wiring_differs_%s' % (cls,)] += 1
                        if [(k, s, p) for k, s, p, _ in templates[cls]['stores']] != [(k, s, p) for k, s, p, _ in stores] or \
                                [t for *_, t in templates[cls]['stores']] != [t for *_, t in stores]:
                            fails['store_templates_differ_%s' % (cls,)] += 1
        print('ctaid.x', c, 'seconds', round(time.time() - t0, 1), flush=True)

    # tid.x independence
    def data(th):
        return [ev[:3] + tuple(x for x in ev[4:] if not isinstance(x, tuple)) for ev in th.events if ev[0] not in ('relread', 'release')]
    ...
    for key in read_keys:
        SRC[key] = np.full((NC, NZ, NY, NL), -9, np.int64)
    for c in range(NC):
        for z in range(NZ):
    ...
    # closed forms written independently from the PTX reading (see RESULT)
    cc, zz, yy, ll = np.meshgrid(np.arange(NC), np.arange(NZ), np.arange(NY), np.arange(NL), indexing='ij')
    qoff = np.array([0, 512, 1024, 1536, 32768, 33280, 33792, 34304])
    closed = collections.OrderedDict()

    def cmp(name, arrs, formula, mask=None):
        good = total = 0
        for key, got in arrs:
            exp = formula(key)
            m = np.ones(got.shape, bool) if mask is None else mask(key)
            good += int(((got == exp) | ~m).sum()); total += int(got.size)
        closed[name] = dict(equal=good, elements=total)

    cmp('sread_src', [(k, SRC[k]) for k in read_keys],
        lambda k: np.where(yy < 2, 65536 * (k[2] // 2) + 16384 * zz + 1024 * k[1] + 512 * (k[2] % 2) + 16 * ll, -1))
    cmp('gload', [(k, OFF[k]) for k in OFF if k[0] == 'gload'],
        lambda k: 1048576 * zz + 65536 * (k[1] + 1) + 2048 * (yy % 2) + 4096 * cc + qoff[k[2]] + 16 * ll)
    cmp('cg_residual', [(k, OFF[k]) for k in OFF if k[0] == 'cg'],
        lambda k: np.where((zz == 0) & (yy < 2), 16384 * (k[2] // 2) + 512 * (k[2] % 2) + 1024 * (yy % 2) + 2048 * cc + 16 * ll, -1))
    cmp('g32_scale', [(k, OFF[k]) for k in OFF if k[0] == 'g32'],
        lambda k: np.where(zz == 0, 4194304 + 4 * (64 * cc + 32 * (yy % 2) + 4 * ((k[2] % 16) // 2) + ll % 4), -1))
    buf_formula = lambda k: np.where(yy < 2, 32768 * (k[2] // 4) + 2048 * (yy % 2) + 4096 * cc + 512 * (k[2] % 4) + 16 * ll, -1)  # noqa: E731
    cmp('bufstore_z0', [(k, OFF[k]) for k in OFF if k[0] == 'bufstore'], lambda k: np.where(zz == 0, buf_formula(k), -1))
    cmp('red_z1_z2', [(k, OFF[k]) for k in OFF if k[0] == 'red'], lambda k: np.where((zz == 1) | (zz == 2), buf_formula(k), -1))
    cmp('bufload_z3', [(k, OFF[k]) for k in OFF if k[0] == 'bufload'], lambda k: np.where(zz == 3, buf_formula(k), -1))
    cmp('outstore_z3', [(k, OFF[k]) for k in OFF if k[0] == 'outstore'],
        lambda k: np.where((zz == 3) & (yy < 2), 16384 * (k[2] // 2) + 1024 * (yy % 2) + 2048 * cc + 512 * (k[2] % 2) + 16 * ll, -1))

    def cover(kind, size, zsel):
        ...
        for k in OFF:
            ...
            a = OFF[k][:, zsel, :2]
            a = a[a >= 0]
            for b in range(16 if kind != 'g32' else 4):
                np.add.at(cnt, a + b, 1)
        return cnt
    cov = {}
    out_c = cover('outstore', 65536, [3]); cov['output_exactly_once'] = [int((out_c == 1).sum()), 65536]
    for kind, zs in (('bufstore', [0]), ('red', [1]), ('red', [2]), ('bufload', [3])):
        cnt = cover(kind, 131072, zs)
        cov['buffer_%s_z%s_exactly_once' % (kind, zs[0])] = [int((cnt == 1).sum()), 131072]
    wc = np.zeros(4194304 + 2048, np.int32)
    for k in OFF:
        if k[0] == 'gload':
            a = OFF[k][:, :, :2]
            for b in range(16):
                np.add.at(wc, a.ravel() + b, 1)
    cov['weights_matrix_read_exactly_once_by_live_warps'] = [int((wc[:4194304] == 1).sum()), 4194304]
    sc = cover('g32', 4194304 + 2048, [0])[4194304:]
    cov['scale_words_read_counts_live_warps'] = sorted(set(sc.tolist()))
    rc = cover('cg', 65536, [0])
    cov['residual_bytes_read_counts_live_warps'] = sorted(set(rc.tolist()))
    ic = np.zeros(262144, np.int32)
    for k in read_keys:
        a = SRC[k][:, :, :2]
        a = a[a >= 0]
        for b in range(16):
    ...
    mma_per_epoch = {str(z): dict(collections.Counter(wr[0] for wr in templates[(z, 'live')]['wiring'])) for z in range(NZ)}
    closed_ok = all(v['equal'] == v['elements'] for v in closed.values())
    cov_ok = (all(v[0] == v[1] for k, v in cov.items() if isinstance(v, list) and len(v) == 2 and k != 'scale_words_read_counts_live_warps'
                  and k != 'residual_bytes_read_counts_live_warps' and k != 'input_bytes_read_counts_live'))
    ok = (sum(fails.values()) == 0 and sum(replay.values()) == 0 and tidx_independent and closed_ok and cov_ok
          and all(len(v) == 16 and all(n == 64 for n in v.values()) for v in mma_per_epoch.values()))
    rep = dict(format='opus-b31-l60-address-map/1', valid=ok, match=None, status='PASS_EXECUTABLE_MAP' if ok else 'MAP_CHECK_FAILED',
               ptx_sha256=fsha(PTX), geometry=dict(grid=grid, block_dim=block, threads=NC * NZ * NY * NL),
               ...
               releases=[dict(z=k[0], y=k[1], region=k[2], offset=k[3], value=k[4], count=n) for k, n in sorted(releases.items())],
               release_reads=[dict(z=k[0], epoch=k[1], region=k[2], offset_minus_4c=k[3], count=n) for k, n in sorted(relreads.items())],
               ...
               hypotheses=['H_TID_X_ZERO', 'H_ELECT_ANY', 'H_BARRIER_VIS', 'H_BRACE_LE', 'H_STAGE_ORDER'],
